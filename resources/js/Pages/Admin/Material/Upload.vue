    ...
    <div class="p-2 px-8">
      <div class="px-4 py-4 sm:px-6 md:px-8">
        <form class="mx-auto mt-8 max-w-lg" @submit.prevent="uploadCSV">
          <p class="mb-6">
            Import multiple cost codes at one time. The cost codes file must be
            in the format shown in the
            <a
              href="/example/timesheets.xlsx"
              class="text-blue-500 hover:underline"
              >csv template</a
            >
            . There should be 7 columns in the file 'Code', 'Description',
            'Unit', 'Category 1 Optional', 'Category 2 Optional', 'Category 3
            Optional', 'Category 4 Optional'. If your file has the same code as
            an existing code the data will be updated.
          </p>
          <p class="mb-2"></p>
...
const crumbspage = ref([
  ...
  { label: "Material", href: "/material-items" },
  { label: "Upload", href: "/materialitems/create" },
]);
...
const uploadCSV = () => {
  ...
  if (csvFile.value) {
    formData.append("csvFile", csvFile.value); // Append the file object to the form data
  }

  // Use Inertia's post method and pass the FormData object
  router.post(route("material-items.import"), formData, {
