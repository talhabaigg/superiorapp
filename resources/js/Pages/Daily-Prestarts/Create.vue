    ...
    <div class="max-w-xl mx-auto m-4 p-8 rounded-lg">
      <form @submit.prevent="submitForm">
        ...
        <div class="flex justify-start items-center space-x-2">
          <PrimaryButton type="submit">Create daily pre-start</PrimaryButton>
...
const crumbspage = ref([
  ...
  { label: "Create", href: "/daily-prestarts/create" },
]);
const selectedProject = ref("");
const selectedForeman = ref("");

const activities = ref([{ name: "" }]);
...
const form = useForm({
  prestart: {
    workdate: new Date().toISOString().substr(0, 10), // Default to today's date
    foreman_id: "",
    weather: "",
    weather_impact: "",
    project_id: "", // Added project_id here since it's part of prestart
  },
  activities: [],
  incidents: [],
});
...
const submitForm = () => {
  // Set the current workdate if needed (this line may not be necessary if already set in useForm)

  // // Set selected values
  // form.prestart.project_id = selectedProject.value; // Assuming selectedProject is a ref
  // form.prestart.foreman_id = selectedForeman.value; // Fixed typo, should be part of prestart

  // Map the activities
  form.activities = activities.value.map((activity) => ({
    name: activity.name, // Assuming activity.name exists
  }));

  // Map the incidents
  form.incidents = incidents.value.map((incident) => ({
    name: incident.name, // Assuming incident.name exists
  }));

  // Submit the form to the Laravel backend
  form.post(route("daily-prestarts.store"));
};
</script>